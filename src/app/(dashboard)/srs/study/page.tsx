...
import { fetchCustomSRSItems, saveSRSProgress } from '@/lib/supabase/data-service'
...
function SRSStudyContent() {
  ...
  const loadItems = useCallback(async () => {
    setLoading(true)
    setErrorMsg('')
    const customItems = await fetchCustomSRSItems(setIds, modes)
    
    // Shuffle items to mix different sets
    const shuffled = customItems.sort(() => 0.5 - Math.random())
    
    setItems(shuffled)
    setCurrentIndex(0)
    setIsAnswerRevealed(false)
    setIsCompleted(shuffled.length === 0)
    setLoading(false)
  }, [setIdsParam, modesParam]) // Dependencies on the raw string params to avoid infinite loops
  ...
  if (loading) {
    return (
      <div className="py-16 text-center text-slate-400 flex flex-col items-center gap-3">
        ...
        <span className="text-xs">Đang tải thẻ từ vựng từ hệ thống...</span>
      </div>
    )
  }
      ...
      <div className="max-w-xl mx-auto py-12 text-center space-y-6">
        <div className="glass-panel p-8 rounded-3xl border border-emerald-500/30 shadow-2xl space-y-6">
          ...
          <div className="space-y-2">
            <h3 className="text-2xl font-black text-white">Bạn đã hoàn thành toàn bộ bài ôn tập! 🎉</h3>
            <p className="text-xs text-slate-300 leading-relaxed">
              Dữ liệu của bạn đã được lưu lại thành công.
            </p>
          </div>
          {reviewedCount > 0 && (
            <div className="p-4 rounded-2xl bg-purple-500/10 border border-purple-500/20">
              <div className="text-2xl font-black text-purple-300">{reviewedCount} Từ Vựng</div>
              <div className="text-xs text-slate-400 mt-1">Đã được học trong phiên này</div>
            </div>
          )}
...
export default function CustomSRSStudyPage() {
  return (
    <Suspense fallback={<div className="py-16 text-center text-slate-400 flex justify-center"><Loader2 className="w-8 h-8 text-purple-400 animate-spin" /></div>}>
      <SRSStudyContent />
    </Suspense>
  )
}

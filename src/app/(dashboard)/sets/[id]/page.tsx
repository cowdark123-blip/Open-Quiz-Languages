...
import {
  fetchVocabSetById,
  fetchVocabItems,
  fetchVocabItemsWithSRSProgress,
  insertVocabItem,
...
export default function SetDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Load data with SRS progress - memoized to prevent infinite loops
  const loadDataWithProgress = useCallback(async () => {
    ...
    try {
      ...
      // Load items WITH SRS progress
      const itemsList = await fetchVocabItemsWithSRSProgress(setId)
      setCurrentSet(setObj)
      setItems(itemsList as VocabItem[])
    } catch (err) {
      console.error('Error loading data with progress:', err)
    } finally {
      setLoading(false)
    }
  }, [setId, router])
  ...
  // Re-sync when page becomes visible (user returns from Flashcard)
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        console.log('🔄 Page visible, re-syncing SRS progress...')
        loadDataWithProgress()
      }
    }
  ...
  const handleBulkImport = async (importedItems: Partial<VocabItem>[], overwrite?: boolean) => {
    if (overwrite) {
      ...
      const freshItems = await fetchVocabItemsWithSRSProgress(setId)
      setItems(freshItems as VocabItem[])
      return true
    } else {
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 text-white text-xs font-bold shadow-sm disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                  >
                    {aiLoading ? (
                      <>
            ...
            <form onSubmit={handleSaveSetInfo} className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
                ...
                <button
                  type="submit"
                  disabled={setSaving}
                  className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white font-bold text-xs shadow-lg shadow-purple-500/25 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >

...
function SetsPageContent() {
  ...
  useEffect(() => {
    setTrashIds(JSON.parse(localStorage.getItem('openquiz_trash_ids') || '[]'))
  }, [sets])
  ...
  const handleDeleteSet = async (id: string) => {
    const trashIds = JSON.parse(localStorage.getItem('openquiz_trash_ids') || '[]')
    if (!trashIds.includes(id)) {
      trashIds.push(id)
      localStorage.setItem('openquiz_trash_ids', JSON.stringify(trashIds))
  ...
  const filteredSets = activeSets.filter((set) => {
    const matchesSearch =
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
        ...
        <div>
          <h2 className="text-2xl font-extrabold text-white tracking-tight flex items-center gap-2">
            ...
            <span className="text-xs px-2.5 py-0.5 rounded-full bg-purple-500/20 text-purple-300 border border-purple-500/30">
              {activeSets.length} bộ
            </span>
          </h2>
          <p className="text-sm text-slate-400 mt-1">
            Tạo và tổ chức các bộ flashcard của riêng bạn.
          </p>
        </div>

        <div className="flex items-center gap-3">
          {activeSets.length === 0 && !loading && (
            <button
      ...
      {/* Filters */}
      <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between bg-slate-900/40 p-4 rounded-2xl border border-slate-800/60">
        <div className="relative w-full md:w-96">
          ...
          <input
            ...
            className="w-full bg-slate-950/50 border border-slate-800 rounded-xl py-2.5 pl-10 pr-4 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 transition-all"
          />
        </div>
      ...
      {contextLoading ? (
        ...
        <div className="py-16 glass-panel rounded-3xl border border-slate-800 text-center space-y-4 max-w-lg mx-auto">
          ...
          <h3 className="text-lg font-bold text-white">Trống Danh Mục Này</h3>
          <p className="text-xs text-slate-400">
            Chưa có bộ từ vựng nào thuộc danh mục <span className="text-purple-400 font-bold">{selectedCategory}</span>.
          </p>
          <div className="flex items-center justify-center pt-2">
            <button
              onClick={() => {
                handleOpenCreateSet()
                setNewCategory(selectedCategory === 'All' ? 'IELTS' : selectedCategory)
              }}
              className="px-4 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-500 text-white font-bold text-xs shadow-md flex items-center gap-2"
            >
              <Plus className="w-4 h-4" />
              Tạo Bộ Từ Mới
            </button>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      onClick={() => {
                        if (window.confirm('Xóa bộ từ này?')) {
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center sm:p-4 bg-black/70 backdrop-blur-sm animate-fade-in overflow-y-auto pb-safe pt-4">
          <div className="w-full sm:max-w-lg glass-panel p-6 rounded-t-3xl sm:rounded-3xl border border-slate-800 shadow-2xl relative mt-auto sm:mt-0 mb-0 pb-12 sm:pb-6">

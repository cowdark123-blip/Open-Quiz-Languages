...
import { BookOpen, Brain, Trophy, Calendar, Sparkles, Layers } from 'lucide-react'
...
export function WordSetCard({ set, learnedCount = 0 }: WordSetCardProps) {
  ...
  const totalItems = set.item_count || 10
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-2 pt-3 border-t border-slate-800/80">
        ...
        <Link
          ...
          className="py-2.5 px-2 rounded-xl bg-slate-900/90 hover:bg-cyan-600/30 text-slate-300 hover:text-cyan-200 border border-slate-800 hover:border-cyan-500/40 text-[11px] font-bold text-center transition-all flex flex-col items-center gap-1 shadow-sm"
        >

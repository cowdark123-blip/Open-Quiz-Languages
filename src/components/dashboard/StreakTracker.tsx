...
export function StreakTracker({ initialStreak }: StreakTrackerProps) {
  ...
  // Generate 28-day history with streak-based highlighting
  const generateHistoryDays = () => {
    ...
    for (let i = 27; i >= 0; i--) {
      ...
      // Mark active if: in activeDates OR within current streak range
      const daysAgo = i
      const isInStreakRange = daysAgo <= (streakCount - 1)
      const isActive = activeDates.includes(dateStr) || isInStreakRange

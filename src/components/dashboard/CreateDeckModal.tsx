...
import { Plus, Upload, X, Loader2, Sparkles, FolderPlus, BookOpen, Layers } from 'lucide-react'
...
export function CreateDeckModal({ isOpen, onClose, initialMode = 'create' }: CreateDeckModalProps) {
  ...
  const [tab, setTab] = useState<'create' | 'import'>(initialMode)
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (newSet) {
      if (tab === 'import' && importText.trim()) {
        // Parse simple term:definition lines
        const lines = importText.split('\n').filter((l) => l.trim().length > 0)
        const items = lines.map((line) => {
          const parts = line.split(/[:-]/)
          const term = parts[0]?.trim() || 'Vocabulary'
          const def = parts.slice(1).join(':').trim() || 'Meaning definition'
          return {
            set_id: newSet.id,
            term,
            definition: def,
            vietnamese_translation: def,
          }
        })
        if (items.length > 0) {
          await insertVocabItemsBatch(items)
        }
      }
      await refreshVocab()
      setTitle('')
          ...
          >
            ...
            <div className="flex rounded-2xl bg-slate-900 p-1 border border-slate-800">
              ...
              >
                ...
                <span>Nhập Hàng Loạt (Import)</span>
              </button>
            </div>
              ...
              {tab === 'import' && (
                <div>
                  <label className="block text-xs font-bold text-slate-300 mb-1.5 flex items-center justify-between">
                    <span>Dán danh sách từ (Mỗi dòng: Từ - Định nghĩa)</span>
                    <span className="text-[10px] text-purple-400 font-mono">Resilience: Khả năng phục hồi</span>
                  </label>
                  <textarea
                    value={importText}
                    ...
                    className="w-full p-3 rounded-xl bg-slate-900 border border-slate-700 font-mono text-xs text-white placeholder-slate-500 focus:outline-none focus:border-purple-500 resize-none"
                  />
                </div>
              )}

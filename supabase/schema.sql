...
ALTER TABLE public.vocab_sets ADD COLUMN IF NOT EXISTS item_count INT DEFAULT 0;
...
-- Vocab Sets: CHỈ ĐỌC/XÉT/SỬA/XÓA BỘ TỪ CỦA CHÍNH MÌNH
DROP POLICY IF EXISTS "Vocab Sets SELECT" ON public.vocab_sets;
CREATE POLICY "Vocab Sets SELECT" ON public.vocab_sets FOR SELECT USING (auth.uid() = user_id);
...
-- Vocab Items: CHỈ ĐỌC items của bộ từ thuộc về mình
